#!/usr/bin/env python3
"""
AWS Spot Instance Blacklist Script (Containerized Version)
...
Environment Variables:
  ...
  INTERRUPTION_THRESHOLD  - Interruption percentage threshold (e.g., 10)
  API_KEY                 - CAST.ai API key
  ORG_ID                  - CAST.ai organization ID
  CLUSTER_ID              - CAST.ai cluster ID
  BLACKLIST_HOURS         - Hours to blacklist instances for (default: 5)
  DRY_RUN                 - Set to "true" for dry run mode
  BATCH_SIZE              - Number of instances to blacklist in a batch (default: 10)
  BATCH_PAUSE             - Seconds to pause between batches (default: 5)
  AUTO_APPROVE            - Set to "true" to skip confirmation (useful for containers)

Command-line arguments are also supported and will override environment variables.
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--os", 
        default=os.environ.get("OS"),
        help="Filter results by operating system (e.g., Linux, Windows)"
    )
    parser.add_argument(
        "--interruption-threshold", 
        type=float,
        default=float(os.environ.get("INTERRUPTION_THRESHOLD", "0")),
    ...
    parser.add_argument(
        "--blacklist-hours",
        type=int,
        default=int(os.environ.get("BLACKLIST_HOURS", "5")),
        help="Number of hours to blacklist instances for (default: 5)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        default=os.environ.get("DRY_RUN", "").lower() in ("true", "yes", "1"),
    ...
    parser.add_argument(
        "--batch-pause",
        type=int,
        default=int(os.environ.get("BATCH_PAUSE", "5")),
        help="Seconds to pause between batches"
    )
    parser.add_argument(
        "--auto-approve",
        action="store_true",
        default=os.environ.get("AUTO_APPROVE", "").lower() in ("true", "yes", "1"),
        help="Skip confirmation prompt (useful for containers)"
    )
    ...
    missing_params = []
    if not args.region:
        missing_params.append("--region or REGION")
    if not args.os:
        missing_params.append("--os or OS")
    if args.interruption_threshold <= 0:
        missing_params.append("--interruption-threshold or INTERRUPTION_THRESHOLD")
    if not args.api_key:
        missing_params.append("--api-key or API_KEY")
    if not args.org_id:
        missing_params.append("--org-id or ORG_ID")
    if not args.cluster_id:
        missing_params.append("--cluster-id or CLUSTER_ID")
...
def main():
    ...
    # Confirm before proceeding if not in dry-run mode and not auto-approved
    if not args.dry_run and not args.auto_approve:
        try:
            ...
            logger.info("No input available or operation cancelled. Use --auto-approve for non-interactive environments.")
            return
